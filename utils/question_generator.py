...
position = set(['in','on','under','below','above','over','on','at','behind','near'])
...
class QuestionGenerator:
    ...
    def askWhere(self, root):
        ...
        def traverse(node, parent):
            ...
            if node.className == 'PP' and \
                    node.children[0].text in position:
                c = node
    ...
    def askAction(self,root):
        root = self.clearCC(root)
        ...
        def traverse(node):
            if found[0]:
                return
            for child in node.children:
                if child.className == 'VBG' or child.className == 'VBP' or child.className == 'VBZ':
                    ind = node.children.index(child)
                    length = len(node.children)
                    if ind < length - 1 and (node.children[ind + 1].className == 'NP' or
                        node.children[ind + 1].className == 'PP'):
                        if child.className == 'VBG':
                            node.children.insert(node.children.index(child),
                                           TreeNode('DOING', 'doing', [], 0))
                        else:
                            node.children.insert(node.children.index(child),
                                                 TreeNode('DO', 'do', [], 0))
            ...
            if found[0]:
                ...
                for i in reversed(range(length)):
                    if items[i].className == 'VBG' or items[i].className == 'NP' or \
                                    items[i].className == 'VBP' or items[i].className == 'VBZ':
                        ans.append(items[i].toSentence().lower())
                        node.children.remove(items[i])
                answer[0] = ' '.join(list(reversed(ans)))
                answer[1] = ans[-1]
            else:
                for child in node.children:
                    traverse(child)
        ...
        if found[0]:
            ...
            ques = 'what does ' + root.toSentence().lower()
            ques = self.trans_ques(ques)
            return [(ques,answer[1],answer[0])]
        else:
            return []

    def trans_ques(self,ques):
        words = ques.split()
        if 'is' in words:
            words.remove('is')
            words[1] = 'is'
    ...
    def clearCC(self,root):
        def traverse(parent,node):
            if node.className == 'CC':
                ...
                for i in reversed(range(length)):
                    if i >= ind:
                        parent.children.remove(childs[i])
...
def printQAs(qa_list,qaiter, qid=0):
    """Print QA pair."""
    for qaitem in qaiter:
       qa_list.append([qid, qaitem[0], qaitem[1],qaitem[2]])
    return qa_list
...
def runSentence(parserFolder, sentence):
    ...
    # qa_list = printQAs(qa_list,qaiter,  qid=0)
    # qaiter = gen.askHowMany(tree.copy())
    # qa_list = printQAs(qa_list, qaiter, qid=1)
    # qaiter = gen.askColor(tree.copy())
    # qa_list = printQAs(qa_list, qaiter, qid=2)
    # qaiter = gen.askWhere(tree.copy())
    # qa_list = printQAs(qa_list, qaiter, qid=3)
    qaiter = gen.askAction(tree.copy())
    qa_list = printQAs(qa_list, qaiter, qid=4)
    print(qa_list)
    return qa_list
...
def parseArgs():
    ...
    parser.add_argument(
        '-parser_path',
        default='/home/zhangzhu/data/parser',
